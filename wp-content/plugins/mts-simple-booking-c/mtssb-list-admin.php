...
class MTSSB_List_Admin extends MTSSB_Booking {
    ...
    private $the_start_date = 0;    // 検索開始日のunix time
    private $the_end_date = 0;		// 検索終了日のunix time
	private $action = '';		// none or montly or between
	...
	public function list_page() {
        ...
        $this->the_start_date = mktime(0, 0, 0, date_i18n('n'), 1, date_i18n('Y'));
        $this->the_end_date = mktime(23, 59, 59, date_i18n('n') + 1, 0, date_i18n('Y'));
		...
		switch ($this->action) {
			case 'monthly' :
				...
				break;
            case 'between' :
                if ((isset($_GET['start_year']) && isset($_GET['start_month']) && isset($_GET['start_day'])) && (isset($_GET['end_year']) && isset($_GET['end_month']) && isset($_GET['end_day']))) {
                    $this->the_start_date = mktime(0, 0, 0, intval($_GET['start_month']), intval($_GET['start_day']), intval($_GET['start_year']));
                    $this->the_end_date = mktime(0, 0, 0, intval($_GET['end_month']), intval($_GET['end_day']), intval($_GET['end_year']));
                }
                break;
			case 'delete' :
				// NONCEチェックOKなら削除する
...
?>
	<div id="schedule-select-article">
		...
		<form method="get" action="">
            ...
            <?php if ($this->action == 'between') { ?>
                開始<?php _e('Year: ', $this->domain); ?>
                <select class="select-year" name="start_year">
                ...
                <select class="select-month" name="start_month">
                    <?php for ($m = 1; $m <= 12; $m++) {
                        echo "<option value=\"$m\"";
                        if ($m == $this_month) {
                            echo ' selected="selected"';
                ...
                <?php _e('Day:',$this->domain); ?>
                <select class="select-day" name="start_day">
                    <?php for ($d = 1; $d <= 31; $d++) {
                        echo "<option value=\"$d\"";
                        if ($d == $this_day) {
                            echo ' selected="selected"';
                ...
                〜
                終了<?php _e('Year: ', $this->domain); ?>
                <select class="select-year" name="end_year">
                    <?php for ($y = $this_year - 1; $y <= $this_year; $y++) {
                        echo "<option value=\"$y\"";
                        if ($y == $this_year) {
                            echo ' selected="selected"';
                ...
                <?php _e('Month:',$this->domain); ?>
                <select class="select-month" name="end_month">
                    <?php for ($m = 1; $m <= 12; $m++) {
                        echo "<option value=\"$m\"";
                        if ($m == $this_month) {
                            echo ' selected="selected"';
                ...
                <?php _e('Day:',$this->domain); ?>
                <select class="select-day" name="end_day">
                    <?php for ($d = 1; $d <= 31; $d++) {
                        echo "<option value=\"$d\"";
                        if ($d == $this_day) {
                            echo ' selected="selected"';
...
<?php
	...
	public function read_data($offset, $limit, $order) {
        ...
        } elseif ($this->action == 'between') {
            $conditions = 'booking_time>=' . mktime(0, 0, 0, date('n', $this->the_start_date), date('j', $this->the_start_date), date('Y', $this->the_start_date))
                . ' AND booking_time<=' . mktime(23, 59, 59, date('n', $this->the_end_date), date('j', $this->the_end_date), date('Y', $this->the_end_date));
            return $this->get_booking_list($offset, $limit, $order, $conditions);
